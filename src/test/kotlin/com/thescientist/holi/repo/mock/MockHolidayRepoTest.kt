...
internal class MockHolidayRepoTest {
    ...
    internal fun `should add new holiday`() {
        ...
        repo.addHoliday(newHoliday)
        assertThat(repo.retrieveHoliday(newHoliday.name)).matches { it == newHoliday }
    }
    ...
    internal fun `should throw exception when holiday already exists`() {
        ...
        assertThrows<IllegalArgumentException> { MockHolidayRepo().addHoliday(existingHoliday) }
    }
    ...
    internal fun `should throw exception when holiday start is within other holiday`() {
        ...
        assertThrows<IllegalArgumentException> { MockHolidayRepo().addHoliday(holiday) }
    }
